...
//
// It incorporates a dispatch table based on the agent.Node values (see the
// agent package for more information) that are yielded from the agent itself.
// Calling NewBuilder with the BuildOpts struct can be used to customize the
// experience for execution purposes only. Parsing is controlled in the agent
...
// Child[Node, Node, Node] where Child is from agent.Node.Children and each
// node comes from agent.Node.Next. This forms a "line" with a statement and
// arguments and we process them in this normalized form by hitting
// evaluateTable with the leaf nodes of the command and the Builder object.
//
// ONBUILD is a special case; in this case the agent will emit:
...
func (b *Builder) dispatch(stepN int, stepTotal int, node *parser.Node, shlex *ShellLex) error {
	...
	// XXX yes, we skip any cmds that are not valid; the agent should have
